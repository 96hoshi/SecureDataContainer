...
public class MySecureDataContainer<E> implements SecureDataContainer<E> {
	 ...
	 *  
	 *  Rep Invariant:  forall i. 0 <= i < users.size => (user_i.getId != null && user_i.getPassw != null && user_i.getDataCollection.getData != null) 
	 ...
	 *  						user_i.dataCollection.getData != null per ogni elemento di dataCollection	
			...
			if(toRemove != null) {
				...
				if(toRemove.getOwner().equals(owner)) {	
					...
					for(String otherUser : toRemove.getOtherCollection()) {
						...
						User userShared = searchUser(otherUser);
	...
	public void share(String owner, String passw, String other, E data) throws Exception{	
		...
		if (user.checkPassw(passw)) {
			...
			if (sharedData.getOwner().equals(owner)) { 
				...
				User userOther = searchUser(other);
	...
	private User searchUser(String id) throws IllegalArgumentException {
